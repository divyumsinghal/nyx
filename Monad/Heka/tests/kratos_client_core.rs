...
use heka::types::{KratosIdentity, KratosIdentityTraits, KratosSession};
use nun::IdentityId;

#[derive(Clone)]
struct MockProvider {
    session_results: Arc<Mutex<VecDeque<Result<KratosSession, KratosProviderError>>>>,
    identity_results: Arc<Mutex<VecDeque<Result<KratosIdentity, KratosProviderError>>>>,
}

impl MockProvider {
    fn with_session_result(result: Result<KratosSession, KratosProviderError>) -> Self {
        Self {
    ...
    fn with_identity_result(result: Result<KratosIdentity, KratosProviderError>) -> Self {
        Self {
...
impl KratosProvider for MockProvider {
    async fn fetch_session(
        &self,
        _session_token: &str,
    ) -> Result<KratosSession, KratosProviderError> {
    ...
    async fn fetch_identity(&self, _identity_id: &str) -> Result<KratosIdentity, KratosProviderError> {
...
async fn validate_session_returns_identity_for_valid_session() {
    ...
    let provider = MockProvider::with_session_result(Ok(KratosSession {
...
async fn get_identity_maps_malformed_provider_id_to_invalid_response() {
    ...
    let provider = MockProvider::with_identity_result(Ok(KratosIdentity {
