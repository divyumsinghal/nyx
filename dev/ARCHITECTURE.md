...
**Nyx = platform services + library crates.** They are the building blocks.
...
| -------------------------------------- | ----------------------------------------------------- | --------------------------- |
| User registration, login, 2FA          | Yes  — every app needs auth                           | `platform/Heka`             |
| API gateway, routing, rate limits      | Yes                                                   | `platform/Heimdall`         |
| Photo/video upload and processing      | Yes                                                   | `platform/nyx-media`        |
| Direct messaging (1:1, group)          | Yes                                                   | `platform/Ogma`             |
| Push + in-app notifications            | Yes                                                   | `platform/Ushas`            |
| Full-text search, geo search           | Yes                                                   | `platform/nyx-search`       |
| App-scoped aliases + privacy isolation | Yes  — core to the multi-app model                    | `platform/Heka`             |
...
| nyx-media worker | —    | Media processing (NATS)            |
...
```
          ...
          ┌──────┬──────┬──────┼──────┬──────┬──────┐
          ▼      ▼      ▼      ▼      ▼      ▼      ▼
       nyx-db  nyx-   nyx-   nyx-   nyx-   nyx-   nyx-
          ...
          ┌──────────────┼──────────────┐
          ▼              ▼              ▼
     nyx-media     Ushas     Heimdall     ← platform binaries
          │
          ├──────┬──────┬──────┬──────┐
          ▼      ▼      ▼      ▼      ▼
...
No cycles. Clean layers. Platform does the heavy lifting. Apps compose and specialize.
...
- **thiserror + anyhow (chosen)**: `thiserror` for typed errors in library crates (platform/), `anyhow` for ergonomic errors in binary crates (apps/). This is the Rust community standard pairing.
