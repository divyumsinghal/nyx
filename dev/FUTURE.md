...
This is the most important deployment file. It starts every piece of infrastructure the Nyx platform depends on. Any developer runs `docker compose -f infra/compose/infra.yml up -d` and has the full platform infrastructure running locally.
...
Usage: `docker build --build-arg BIN_NAME=Uzume -f infra/docker/Dockerfile.service .`

### infra/config/kratos/identity.schema.json
...
Migrations are plain SQL files, run by `nyx-xtask migrate`. The `nyx-db` crate discovers migration files by scanning these directories and runs them in order using `sqlx::migrate!`. Each app's migrations are isolated to their own schema — they cannot accidentally affect another app's tables.
