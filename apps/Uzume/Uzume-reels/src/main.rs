use tracing::info;
use uzume_reels::{config, routes, state::AppState, workers};
...
async fn main() -> anyhow::Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "uzume_reels=info,nyx_api=info".into()),
        )
        .json()
        .init();
    ...
    tokio::spawn(workers::score_updater::run(state.clone()));
